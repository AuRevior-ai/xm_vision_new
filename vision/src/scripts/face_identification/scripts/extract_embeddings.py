# import libraries
# 此脚本用于从图像数据集中提取面部嵌入并将其序列化以供后续训练使用。
...
protoPath = os.path.join(current_dir, "..", "face_detection_model", "deploy.prototxt")
modelPath = os.path.join(current_dir, "..", "face_detection_model", "res10_300x300_ssd_iter_140000.caffemodel")
detector = cv2.dnn.readNetFromCaffe(protoPath, modelPath)

# load serialized face embedding model
print("Loading Face Recognizer...")
embedder = cv2.dnn.readNetFromTorch(os.path.join(current_dir, "..", "assets", "openface_nn4.small2.v1.t7"))

# grab the paths to the input images in our dataset
print("Quantifying Faces...")
imagePaths = list(paths.list_images(os.path.join(current_dir, "..", "dataset")))
...
f = open(os.path.join(current_dir, "..", "output", "embeddings.pickle"), "wb")
